...
public class openfile {  
	...
	try{  
		...
		if (file.isDirectory())
		{
			//Toast.makeText(getContext(), "not dirctory",Toast.LENGTH_SHORT).show();
	...
	{  
	    if(file.isDirectory() && file.listFiles()!=null){  
	        ...
	        if(suffix == null || suffix.length()==0 || (sf.length()>0 && suffix.indexOf("."+sf+";")>=0)){  
	        	if (!file.getName().contains("_detect.jpg"))
	        	{
		            Map<String, Object> map = new HashMap<String, Object>();  
		            map.put("name", file.getName());  
		            map.put("path", file.getPath());  
	...
	SimpleAdapter adapter = new SimpleAdapter(getContext(), list, R.layout.open_file, new String[]{"img", "name", "path"}, new int[]{ R.id.filedialogitem_name, R.id.filedialogitem_path});  
