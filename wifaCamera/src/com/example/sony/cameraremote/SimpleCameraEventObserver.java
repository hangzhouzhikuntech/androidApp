...
public class SimpleCameraEventObserver {
            ...
            public void run() {
                ...
                MONITORLOOP: while (mWhileEventMonitoring) {
                    ...
                    try {
                        ...
                        System.out.println("getEvent errorCode: "+errorCode);
                        switch (errorCode) {
                            case 0: // no error
                                // Pass through.
                                break;
                        ...
                        //System.out.println("getEvent timeout by client trigger.");
                        fireResponseErrorListener();
                        break MONITORLOOP;
                    } catch (JSONException e) {
                        Log.w(TAG, "getEvent: JSON format error. " + e.getMessage());
                        //System.out.println("getEvent: JSON format error");
                        fireResponseErrorListener();
                        break MONITORLOOP;
                    }
    ...
    private void fireResponseErrorListener() {
        mUiHandler.post(new Runnable() {
            @Override
            public void run() {
            	System.out.println("fireResponseErrorListener post");
                if (mListener != null) {
                	System.out.println("fireResponseErrorListener onResponseError");
